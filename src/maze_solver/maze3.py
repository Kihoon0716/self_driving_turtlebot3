...
class Maze_pathfinder():
    ...
    def callback2(self, odometry):
        print odometry.pose.pose.position.x, odometry.pose.pose.position.y
        ...
        self.theta = tf.transformations.euler_from_quaternion(quaternion)[2] - 3.141592/2
        #self.theta = tf.transformations.euler_from_quaternion(quaternion)[2] ##here
        if self.theta < 0:
            self.theta = self.theta + 3.141592*2
        direction_desired = math.atan2(self.low_position - self.path[0], self.path[1] - self.col_position)
        #direction_desired = math.atan2(self.low_position - self.path[0], self.col_position - self.path[1])##here
    ...
    def tf_listener(self):
        ...
        while not rospy.is_shutdown():
            try:
                ...
                self.tf_map_to_odom = [trans[0], trans[1]]
    ...
    def main(self):
        thread_tf_listener = threading.Thread(target = self.tf_listener)
        thread_tf_listener.start()
        rospy.spin()
